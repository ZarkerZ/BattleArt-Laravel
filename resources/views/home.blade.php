...
<body>
    ...
    <div class="main-banner-area">
        <div class="container hero-section">
            ...
            <p>Discover, create, and share your unique digital art with a vibrant community.</p>
            <a href="{{ Auth::check() ? route('challenges.index') : route('login') }}" class="btn btn-light rounded-pill px-4">Start Creating Now</a>
        </div>
        ...
        <div class="container mt-4">
            ...
            <form action="{{ route('search') }}" method="GET" class="d-flex justify-content-center">
                <div class="input-group" style="max-width: 600px;">
                    <input type="text" name="query" class="form-control rounded-start-pill" placeholder="Search for challenges or users...">
                    <button class="btn btn-light rounded-end-pill border" type="submit"><i class="fas fa-search"></i></button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="container">
        <h2 class="text-center text-primary fw-bold mb-4" style="color: var(--primary-bg)!important;">TRENDING</h2>

        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-4 g-4 justify-content-center">
            @forelse($trending_challenges as $challenge)
                <div class="col">
                    <div class="placeholder-card">
                        <a href="{{ url('challenge/' . $challenge->challenge_id) }}">
                            <img src="{{ asset('assets/uploads/' . $challenge->original_art_filename) }}" class="trending-card-image mb-2">
                        </a>
                        <h5 class="fw-bold">{{ $challenge->challenge_name }}</h5>
                        <p class="text-muted small">by {{ $challenge->user_userName }}</p>

                        <div class="d-flex justify-content-center gap-3 mb-3">
                            <span><i class="fas fa-heart text-danger"></i> {{ $challenge->like_count }}</span>
                            <span><i class="fas fa-comment text-primary"></i> {{ $challenge->comment_count }}</span>
                        </div>

                        @if(Auth::id() == $challenge->user_id)
                            <a href="{{ url('challenge/' . $challenge->challenge_id) }}" class="btn btn-primary rounded-pill btn-sm">View Challenge</a>
                        @else
                            <a href="{{ Auth::check() ? url('interpretation/create?id=' . $challenge->challenge_id) : route('login') }}" class="btn btn-primary rounded-pill btn-sm">Challenge!</a>
                        @endif
                    </div>
                </div>
            @empty
                <p class="text-center text-muted col-12">No trending challenges found yet.</p>
            @endforelse
        </div>
