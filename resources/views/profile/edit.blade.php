...
<body>
    <a href="{{ $profileLink }}" class="absolute top-4 left-4 bg-white bg-opacity-30 backdrop-blur-sm text-white text-sm font-semibold py-2 px-4 rounded-xl shadow-md hover:bg-opacity-40 transition duration-200 flex items-center z-10">
    ...
    <div class="main-container">
        ...
        <form id="profileSettingsForm" action="{{ route('edit-profile.update') }}" method="post" enctype="multipart/form-data" class="px-8 pt-6 pb-8 space-y-8">
            @csrf
                    ...
                    <div class="grid grid-cols-1 gap-4 items-end">
                        ...
                        <div class="relative">
                            ...
                            <div class="flex space-x-2">
                                <input type="email" id="email" value="{{ $user->user_email }}" disabled class="flex-grow px-3 py-2 border border-gray-200 rounded-lg text-sm bg-gray-50 text-gray-500 cursor-not-allowed">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div>
                ...
                <textarea id="userBio" name="userBio" rows="4" oninput="updateCharCount()" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-purple-500 focus:border-purple-500" placeholder="Tell everyone a little about yourself...">{{ old('userBio', $user->user_bio) }}</textarea>
            ...
            @endif
            @if($errors->any())
                <div class='p-3 mb-4 rounded-lg text-sm font-medium bg-red-100 text-red-800'>
            ...
            <div class="pt-4 flex justify-center space-x-4 border-t border-gray-100">
                <button id="saveButton" type="submit" class="flex items-center px-6 py-2 bg-purple-600 text-white font-semibold rounded-xl shadow-lg hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 transition">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 13.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" /></svg>
                    Save Changes
                </button>
                <a href="{{ $profileLink }}" class="flex items-center px-6 py-2 bg-gray-600 text-white font-semibold rounded-xl shadow-lg hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition">
    ...
    <script>
        ...
        profileCropButton.addEventListener('click', () => {
            ...
            // Ensure IMG is shown and SVG is hidden
            const svgIcon = document.querySelector('.profile-img-container svg');
